            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Environment: <strong className="text-foreground uppercase">{health?.environment || 'development'}</strong> &bull; Node: {health?.nodeVersion}
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <CardContent className="space-y-3 text-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono font-semibold text-emerald-400">
                {health?.database.pingLatencyMs} ms
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground text-xs">Connection State:</span>
              <span className="font-mono text-xs text-foreground">readyState {health?.database.readyState}</span>
            </div>
            <div className="text-xs text-muted-foreground break-all pt-2 border-t border-border/50">
              Host: <span className="font-mono text-foreground">{health?.database.connectedHost}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardHeader>
            <CardTitle className="text-base flex items-center justify-between">
              ...
              <Badge variant="outline" className="border-purple-500/30 text-purple-400 bg-purple-500/10">
                {health?.memory.heapUsedMb} MB Used
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border/50 text-xs">
              ...
              <span className="font-mono text-foreground">{health?.memory.rssMb} MB</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent className="space-y-3 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Payment Engine (Razorpay):</span>
              <Badge variant="outline" className="border-emerald-500/30 text-emerald-400 bg-emerald-500/10">
                {health?.integrations.paymentGateway || 'ONLINE'}
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Object Storage (ImageKit CDN):</span>
              <Badge variant="outline" className="border-emerald-500/30 text-emerald-400 bg-emerald-500/10">
                {health?.integrations.cloudStorage || 'CONNECTED'}
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Background Cron Schedules:</span>
              <Badge variant="outline" className="border-blue-500/30 text-blue-400 bg-blue-500/10">
                {health?.integrations.cronJobs || 'RUNNING'}
              </Badge>
            </div>
          </CardContent>
        </Card>
